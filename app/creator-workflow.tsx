...
function researchForTopic(currentTopic: string, context: any = {}) {
  ...
  const retentionUnits = [
    ...
    `单元五｜摆出最强反方和可观察条件，给出当前判断；结尾明确总结，并把案例压缩成观众下次可复用的分析顺序。`,
  ];
  return [
    {
    ...
    },
    {
      ...
      body: `开头先用“${clean.slice(0, 34)}”的异常制造困惑，并迅速给出一句核心判断方向；中段按解释力从主因到次因展开，在“${coreConcept}”处形成由证据推动的认知转折；结尾明确总结判断，再交付一套类似事件可复用的观察顺序，让观众获得掌控感。情绪服务于事实，不能靠夸张词硬煽动。`,
    },
    {
