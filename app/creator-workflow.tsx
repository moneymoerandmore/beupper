...
function researchForTopic(currentTopic: string, context: any = {}) {
  ...
  return [
    ...
    },
    {
      ...
      body: `先判断本题属于搜索型、推荐型还是双引擎。搜索型必须在开头准确兑现用户会输入的主体、最新动作、冲击数字和时间；推荐型必须让没有主动搜索的人也能立刻理解谁获益、谁承担成本、价格为什么反常；双引擎两者都要做到。历史29条抖音作品的中位播放为1008、中位平均观看约33秒；阿里配售稿平均观看58.76秒、希音破发稿44.33秒，两条的2秒跳出均低于29%、5秒完播均高于51%。因此把前60秒独立设计：前2秒给异常，前5秒交代主体与新动作，前15秒说明和普通股民的关系，前60秒完成第一轮因果闭环。封面点击率不作为正文结构依据。`,
    },
    {
