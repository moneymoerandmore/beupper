...
强制检查事实是否只来自证据池且不伪造、因果是否闭环、标题承诺是否兑现、第一段是否使用当前包装的Hook与核心矛盾、异常之后是否在前220个汉字内给出一句有因果方向的核心判断、全文是否只讲一个核心判断、动态创作底稿是否被正确取舍而不是逐卡扩写、证据是否按解释力排序、跨市场内容是否确有主题依据、反方是否具体且可验证、结尾是否先明确总结并给出可复用观察顺序、随后是否直接说出点赞和关注、自然包含评论互动、持续更新价值与泛化下期承接，且没有具体选题预告，以及是否存在AI腔、空话、重复、裸设问接缝和生硬互动。只报行情、日期、涨跌幅或连涨连跌纪录不算核心判断。发现问题直接重写。宁可删除无依据的细节，也不要补造事实。
...
硬性要求：1000至3000个汉字；第一句进入当前包装锁定的真实事件瞬间与冲突，前220个汉字内必须紧跟一句有因果方向的核心判断，只报行情或涨跌纪录不算判断；全文只有一个核心判断。根据动态研究底稿选择结构：事件型内容可沿“行动—市场反应—新证据—认知转折—后续影响”推进；原因分析型内容围绕同一问题，按解释力从主因到次因逐层下探。不能把观点并排罗列。把核心因果一步步讲成人话；只有主题确有跨市场关联时，才继续解释传到其他市场还缺什么条件。句子长短自然交替，每段集中推进一个信息点，合理使用“我、你、咱们”。全文最多使用一个真正好懂的原创意象。反方必须能用具体信号推翻当前判断。结尾必须先明确总结当前判断，给出类似事件可复用的观察顺序、最关键验证信号和判断失效条件；随后用一至三句完成评论互动，直接说出“点赞”和“关注”，说明持续获得最新财经热点深度分析的价值，并用泛化语言承接下期。互动不得询问持仓、成本和买卖计划，关注引导不得承诺收益；不得预告下一篇的具体公司、事件、数据、市场、观点或题目，也不得虚构排期。禁止报告体连接词、论文句式、新闻播音腔，以及反复使用“你可能以为”“真正的问题是”“不是A而是B”等套路接缝。绝对禁止标题、Markdown、提纲、编号、项目符号、创作思路、情绪弧线、留存单元说明、审稿意见、修改说明、字数统计、开场白或“以下是正文”。绝对不能向用户索要信息、征求是否继续、提出合作建议或解释还缺什么；证据不足就删除、限定或转成验证条件，然后直接完成稿件。
...
def opening_performance_problems(text):
    ...
    if not anomaly.search(first_2s): problems.append("前2秒没有出现具体异常或反预期结果")
    if not event_action.search(first_5s): problems.append("前5秒没有说清本次事件的新动作或价格反应")
    if not investor_relevance.search(first_15s): problems.append("前15秒没有说明这件事与股民或资产价格的关系")
    if not (causal.search(first_60s) and consequence.search(first_60s)): problems.append("前60秒没有完成事件、原因与股价影响的第一轮因果闭环")
    return problems
