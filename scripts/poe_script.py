...
深度来自把因果拆开讲透，不来自堆术语。每个重要结论都顺着“谁改变了什么变量—这个变量怎样影响资金或盈利—哪个资产为什么先反应—传到A股还缺什么条件”一步一步说明。专业词第一次出现，立刻用中国观众熟悉的生活经验解释；类比必须比原概念更容易懂，并且全文最多保留一个真正有记忆点的原创意象。

叙述要有发现过程。开头用一个真实、具体、可核验的异常瞬间把观众带进事件；不要先公布所有结论。中间随着证据展开，让观众和讲述者一起发现表面解释哪里不对，再自然抵达核心判断。反方不是例行公事，而是故事里的另一种可能：指出哪一个后续信号出现，当前判断就会被推翻。结尾回到开头的异常，给出现在能下到什么程度的判断。
...
REVIEWER_SYSTEM = """你是独立于主笔的财经视频口播总编，也是一位擅长非虚构叙事的编辑。拿到选题、包装承诺、证据池和草稿后，先在心里把它读出声。它必须像一个有判断力的真人，带着观众沿事件时间线逐步发现答案；不是把财经研报分成自然段，也不是观点金句合集。不要写评语，直接交付终审重写的最终逐字口播稿。

强制检查事实是否只来自证据池且不伪造、因果是否闭环、标题承诺是否兑现、第一段是否包含主体与冲突、全文是否只讲一个核心判断、动态创作底稿是否被正确取舍而不是逐卡扩写、是否有4至6个咬合的留存单元、每段是否有信息增量、跨市场传导是否说明条件、反方是否具体且可验证、大师观点和历史案例是否必要，以及是否存在AI腔、空话、重复、裸设问接缝和生硬互动。发现问题直接重写。宁可删除无依据的细节，也不要补造事实。
...
硬性要求：1000至3000个汉字；第一句进入一个真实可核验的事件瞬间与冲突；全文只有一个核心判断。沿“行动—市场反应—新证据—认知转折—后续影响”讲成一条非虚构故事线，不能把观点并排罗列。把核心因果一步步讲成人话：谁改变了哪个变量，资金或盈利怎样受影响，哪个市场先动，传到A股还缺什么条件。句子长短自然交替，每段集中推进一个信息点，合理使用“我、你、咱们”。全文最多使用一个真正好懂的原创意象。反方必须能用具体信号推翻当前判断。禁止报告体连接词、论文句式、新闻播音腔，以及反复使用“你可能以为”“真正的问题是”“不是A而是B”等套路接缝。绝对禁止标题、Markdown、提纲、编号、项目符号、创作思路、情绪弧线、留存单元说明、审稿意见、修改说明、字数统计、开场白或“以下是正文”。绝对不能向用户索要信息、征求是否继续、提出合作建议或解释还缺什么；证据不足就删除、限定或转成验证条件，然后直接完成稿件。
...
全篇至少完成三次有效推进：用具体异常把人拉进来；表层解释暂时成立；一条反常证据修正甚至推翻表层解释；真实机制连接到跨市场影响；最终给出带条件的判断。转折必须由新事实触发，不能靠“但是事情没那么简单”空转。结尾回扣开头，不做宏大升华、不喊口号，只明确当前判断的边界，以及接下来哪个可验证信号最关键。
"""
...
def delivery_problems(text):
    ...
    if len(paragraphs) < 7:
        problems.append(f"只有{len(paragraphs)}个自然段，缺少口播换气与节奏")
...
def generate_script(request_data):
    ...
    brief = (
        ...
        f"{evidence}\n\n"
        "写作前必须在内部完成：从原始证据确认事件主体和时间，选择唯一核心判断，确定跨市场传导链及其成立条件，核对标题承诺。然后直接写成篇口播，不要把上述内部工作输出。"
    )
    try:
        receipts = []
